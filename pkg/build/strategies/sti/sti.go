...
import (
	...
	"github.com/magicsong/s2irun/pkg/build/strategies/layered"
	dockerpkg "github.com/magicsong/s2irun/pkg/docker"
	s2ierr "github.com/magicsong/s2irun/pkg/errors"
	"github.com/magicsong/s2irun/pkg/ignore"
	"github.com/magicsong/s2irun/pkg/scm"
	"github.com/magicsong/s2irun/pkg/scm/git"
	"github.com/magicsong/s2irun/pkg/scripts"
...
// For more details about S2I, visit https://github.com/s2iservice
type STI struct {
	config                 *api.Config
	result                 *api.Result
...
func New(client dockerpkg.Client, config *api.Config, fs fs.FileSystem, overrides build.Overrides) (*STI, error) {
	...
	builder := &STI{
		...
		fs:                     fs,
		tar:                    tarHandler,
		callbackInvoker:        utils.NewCallbackInvoker(),
...
func (builder *STI) Build(config *api.Config) (*api.Result, error) {
	...
	if builder.incremental = builder.artifacts.Exists(config); builder.incremental {
		tag := utils.FirstNonEmpty(config.IncrementalFromTag, config.Tag)
...
func (builder *STI) Prepare(config *api.Config) error {
	...
	if len(config.ScriptsURL) > 0 {
		...
		for _, result := range requiredAndOptional {
			if utils.Includes(result.FailedSources, scripts.ScriptURLHandler) {
				failedCount++
			}
		}
...
func (builder *STI) Save(config *api.Config) (err error) {
	...
	image := utils.FirstNonEmpty(config.IncrementalFromTag, config.Tag)
...
func (builder *STI) Execute(command string, user string, config *api.Config) error {
	...
	if len(config.Injections) > 0 && command == constants.Assemble {
		...
		config.Injections = utils.FixInjectionsWithRelativePath(workdir, config.Injections)
		truncatedFiles, err := utils.ListFilesToTruncate(builder.fs, config.Injections)
		if err != nil {
			builder.result.BuildInfo.FailureReason = utilstatus.NewFailureReason(
				utilstatus.ReasonInstallScriptsFailed,
				utilstatus.ReasonMessageInstallScriptsFailed,
		...
		rmScript, err := utils.CreateTruncateFilesScript(truncatedFiles, rmInjectionsScript)
		if len(rmScript) != 0 {
...
func (builder *STI) uploadInjections(config *api.Config, rmScript, containerID string) error {
	...
	for _, s := range config.Injections {
		if err := builder.docker.UploadToContainer(builder.fs, s.Source, s.Destination, containerID); err != nil {
			return utils.HandleInjectionError(s, err)
		}
	}
	if err := builder.docker.UploadToContainer(builder.fs, rmScript, rmInjectionsScript, containerID); err != nil {
		return utils.HandleInjectionError(api.VolumeSpec{Source: rmScript, Destination: rmInjectionsScript}, err)
	}
	return nil
}
...
func (builder *STI) uploadInjectionResult(startErr error, containerID string) error {
	resultFile, err := utils.CreateInjectionResultFile(startErr)
	if len(resultFile) > 0 {
	...
	if err != nil {
		return utils.HandleInjectionError(api.VolumeSpec{Source: resultFile, Destination: injectionResultFile}, err)
	}
	return startErr
}
